...
class ConfirmRegistrationView(View):
    ...
    def get(self, request, department_id, doctor_id, consultation_hours):
        ...
        return render(request, 'confirmregistration.html', {'patient_name': patient_name, 'doctor_name': doctor_name,
                                                            ...
                                                            'address': address, 'tomorrow': tomorrow})
    ...
    def post(self, request):
        ...
        Register.objects.create(patient_name=patient_name, doctor_name=doctor_name, registration_time=registration_time,
        ...
        doctor_time_number = TimeNumber.objects.get(doctor_id=doctor_id)
        consultation_hours_time = consultation_hours[11:]
...
class ShowRegisterView(View):
    ...
    def get(self, request):
        ...
        if identity == 'patient':
            patient = request.session.get('patient')
            register_list = patient.register_set.order_by('id').filter(isdelete=False).all()

            return render(request, 'patientcenter.html', {'register_list': register_list})
        else:
            doctor = request.session.get('doctor')
            register_list = doctor.register_set.order_by('id').filter(isdelete=False).all()

            return render(request, 'doctorcenter.html', {'register_list': register_list})
